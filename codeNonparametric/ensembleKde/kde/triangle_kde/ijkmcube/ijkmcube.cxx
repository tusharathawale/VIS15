...
/// Marching Cubes Algorithm. (without taking into account the data uncertainty) 
...
/// Marching Cubes Algorithm (taking into account the data uncertainty).
// Takes mean (mc_data) and  ensesemble with bandwidth (mc_data_1) as inputs and returns colors proportional to variance of level-crossing location
// change 
void IJKMCUBE::marching_cubes
(const MC_DATA & mc_data, const SCALAR_TYPE isovalue, 
 ...
 MC_ISOSURFACE & mc_isosurface_1, MCUBE_INFO & mcube_info_1, float* &f_color, float* numv)
{
  ...
  // change
  // Ensemble data packed with bandwidth
  ...
  if (mc_data.UseNEP()) {		 
	...
	//This is called in MC without considering data uncertainty
/*	marching_cubes
      (mc_data.ScalarGrid(), mc_data.isotable,
       isovalue, isosurface_topology, interpolation_type,
       mc_isosurface.simplex_vert, mc_isosurface.vertex_coord,
	...
	//This is called for MC by considering data uncertainty
        // Specifically, additional parameters passed are mean (mc_data) and ensemble+kde bandwidth (mc_data_1) data
	marching_cubes
      (mc_data.ScalarGrid(), mc_data.isotable,
       isovalue, isosurface_topology, interpolation_type,
       mc_isosurface.simplex_vert, mc_isosurface.vertex_coord,
...
// Newly added
/// Marching Cubes Algorithm (taking into account the data uncertainty).
/// @param scalar_grid = Mean field
/// @param scalar_grid_1 = ensemble packed together with bandwidth for kernel density estimation (reminder: We prepared
/// mean and packed data using code provided in directory generateInputData)
/// @param isosurface_topology = Isosurface topology.
void IJKMCUBE::marching_cubes
(const MC_SCALAR_GRID_BASE & scalar_grid, const POLY_ISOTABLE & poly_isotable,
 const SCALAR_TYPE isovalue,
 ...
 const MERGE_EDGES_PARAMETERS & merge_parameters_1, MCUBE_INFO & mcube_info_1, float* &f_color, float* numv)
{
  ...
  // Extract polygons per cube from the mean grid. (This determines which grid cubes are intersected by an isosurface and the isosurface topology within each intersected cube and not geometry.) 
 /* extract_iso_simplices
    (scalar_grid, poly_isotable, isovalue, isosurface_topology,
     simplex_vert, endpoint, new_mesh_vertices, mcube_info);*/

  // Extract polygons per cube from the most probable grid. (This determines which cubes and which edges of the cube are intersected by an isosurface and the isosurface topology within each intersected cube and not geometry.)  	
  // For description on computing the most probable grid refer to vertex-based classification technique proposed in Sec. 3.1 of the paper
  extract_iso_simplices
    (scalar_grid, poly_isotable, isovalue, isosurface_topology,
  ...
  // (This determines polygon geometry by finding isosurface crossing location on each cube edge.) 
  // This corresponds to isosurface crossing variability computed on each grid edge using techniques proposed in Sec. 3.2 of the paper.
  merge_and_position_vertices
    (scalar_grid, isovalue, new_mesh_vertices, endpoint, BINARY, 
     interpolation_type, simplex_vert, vertex_coord, 
