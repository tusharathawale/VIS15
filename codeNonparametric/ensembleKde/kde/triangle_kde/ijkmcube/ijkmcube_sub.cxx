...
namespace {
	...
	void equalizeHistogram(int nume, float* intensity, float max_intensity, float* equalized_intensity, float* &f_color)
	{
			float* normalized_variance = new float[nume];
			float* copy_normalized_variance = new float[nume];	
			float* intensity_levels = new float[nume]; 	
			int* count_intensity = new int[nume];
			float l1;
			float l2;
			float l3;
			...
			for (int i = 0; i < nume; i++)
			{
				normalized_variance[i] = (double)(intensity[i]/max_intensity);	
			...
			for (int i = 0; i < nume; i++) 
			{
				...
				if( i == 0)
				{
					//copy the intensity level. Intensity level array stores different intensities in the image.
					// count_intensity contains count of intensity level in the image 	
					intensity_levels[track_level] = copy_normalized_variance[i];
					count_intensity[track_level] = 1;  
				}
				else
				{
					if( fabs(copy_normalized_variance[i] - copy_normalized_variance[i-1]) > DBL_EPSILON )
					{
						track_level++;
						intensity_levels[track_level] = copy_normalized_variance[i];
						count_intensity[track_level] = 1; 
			...
			// Map the variance values in the range 0 to 255
			for (int i = 0; i < nume; i++) 
			{

				for(int j=0; j<=track_level; j++)
				{
					if (fabs(normalized_variance[i] - intensity_levels[j]) < DBL_EPSILON)
						cdf_i = cdf[j];
				}

				normalized_variance[i] = (double)(((cdf_i - cdf_min)*255.0)/(nume - cdf_min));
				cout<< normalized_variance[i] << " ";

			}	
			
			l3 = 127.0;

			//linearly interpolate from green to blue to red
			for (int i = 0; i < nume; i++) 
			{		

				if((normalized_variance[i] >= 0) && (normalized_variance[i]<=127) )
				{
					double t = (double)((normalized_variance[i])/l3);
				...
				else if((normalized_variance[i] > 127) && (normalized_variance[i]<=255))
				{	
			...
			delete [] normalized_variance; 	
			delete [] copy_normalized_variance;
			delete [] intensity_levels;
			delete [] count_intensity;
	...
	void position_iso_vertices_linear_binaryB_alpha
		...
		//                 dimension*vlist.size()
		{
			...
			const int * axis = scalar_grid_1. AxisSize();
			/*  for(int i = 0; i<scalar_grid_1.NumVertices(); i++)
			    cout<<scalar_1[i]<<" ";
			    cout<<"\n";
			...
			//  Get the dimension of vector stored at each space location   
			int dim =  axis[0];   
			...
			// Initialize maximum variance to 0.
			*max_variance = 0;

			for (int i = 0; i < nume; i++) {

				// for kde mean members
				SCALAR_TYPE* s0 = new SCALAR_TYPE[dim-1]; 
				SCALAR_TYPE* s1 = new SCALAR_TYPE[dim-1]; 
				...
				// Get ensemble at one end of the edge
				if (v0 < scalar_grid.NumVertices()) {
					//s0 = scalar[v0];
					for(int i=0; i< dim-1; i++)
					{
						s0[i] = scalar_1[dim*v0 + i];  
					}
					scalar_grid.ComputeCoord(v0, coord0);
				}
				else {
				...
				// Get ensemble at other end of the edge
				if (v1 < scalar_grid.NumVertices()) {
					//s1 = scalar[v1];
					for(int i=0; i< dim-1; i++)
					{
						s1[i] =  scalar_1[dim*v1 + i];  
					}	
					scalar_grid.ComputeCoord(v1, coord1);
				}
				else {
				...
				// Get the bin width at one end of the edge				
				if (v0 < scalar_grid_1.NumVertices()) {
					//s2 = scalar_1[v0];
					s2 = scalar_1[dim*v0 + dim-1];   
					// *** VIMP : Don't compute coordinates here. Keep following line commented
				...
				// Get the bin width at the other end of the edge	
				if (v1 < scalar_grid_1.NumVertices()) {
					//s3 = scalar_1[v1];
					s3 = scalar_1[dim*v1 + dim-1];   
					// *** VIMP : Don't compute coordinates here. Keep following line commented
				...
				// variation in mean is bandwidth/2, dim-1 is number of kernels at each grid point
				kde_alpha_uncertainty_coord(dimension, s0, coord0, s1, coord1, isovalue, coord2, s2, s3, max_variance, color,dim-1);
			...
			equalizeHistogram(nume, normalized_variance, *max_variance, normalized_variance, f_color);
		}
